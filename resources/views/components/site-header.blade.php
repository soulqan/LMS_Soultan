...
<header class="sticky top-0 z-50 border-b {{ $barClasses }}">
    <div class="mx-auto flex min-h-20 max-w-7xl items-center gap-4 px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center gap-2 sm:hidden">
            @guest
                <a href="{{ route('login') }}" class="rounded-xl border px-3 py-2 text-sm font-semibold transition {{ $dark ? 'border-slate-700 text-slate-200 hover:bg-slate-900' : 'border-slate-200 text-slate-700 hover:border-slate-300 hover:bg-slate-50' }}">
                    Sign In
                </a>
            @else
                <div class="relative">
                    <button
                        type="button"
                        data-profile-toggle
                        class="flex max-w-[12rem] items-center gap-2 rounded-xl border px-3 py-2 text-left text-sm font-medium transition {{ $accountButtonClasses }}"
                    >
                        <span class="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-blue-600 text-xs font-semibold text-white">
                            {{ strtoupper(mb_substr($authUser->name, 0, 1)) }}
                        </span>
                        <span class="min-w-0 flex-1">
